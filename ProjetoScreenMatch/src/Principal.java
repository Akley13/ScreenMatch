import br.com.screenmatch.modelos.Titulos;

public class Principal {
    public static void main(String[] args) {

        Titulos meufilmePreferido = new Titulos();
        meufilmePreferido.setNome("Efeito Borboleta");
        meufilmePreferido.setSinopse("O estudante universitário Evan Treborn está aflito com dores de cabeça tão fortes que frequentemente desmaia. Enquanto está inconsciente, Evan pode viajar de volta no tempo para momentos de dificuldades na infância. Ele também pode alterar o passado para os amigos, como Kayleigh, que foi molestada pelo pai. Porém ao mudar o passado, ele pode alterar o presente, o que leva Evan a se encontrar em um pesadelo de realidades alternativas, incluindo uma onde ele está preso.");
        meufilmePreferido.setAnoDeLancamento(2004);
        meufilmePreferido.setDuracaoEmMinutos(120);
        meufilmePreferido.setIncluidoNoPlano(true);

        meufilmePreferido.avaliacao(10);
        meufilmePreferido.avaliacao(8.5);
        meufilmePreferido.avaliacao(9.8);

        meufilmePreferido.exibeFichaTecnica();
    }
}